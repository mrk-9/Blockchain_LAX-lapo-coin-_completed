...
//    unitDisplayControl = new UnitDisplayStatusBarControl();
    ...
    connect(importWalletAction, SIGNAL(triggered()), rpcConsole, SLOT(walletImport()));
    ...
    connect(rpcConsole, SIGNAL(handleRestart(QStringList, int)), this, SLOT(handleRestart(QStringList, int)));
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    importWalletAction->setStatusTip(tr("Import wallet (replaces current)"));
...
void BitcoinGUI::handleRestart(QStringList args, int action)
{
    if (action == 1) {
        if (!walletFrame)
            return;
            
        // use bitcoinGUI to add wallet location
        QString dest = walletFrame->getWalletPath();
        if (dest.trimmed().isEmpty())   // this shouldn't happen
            return;
    ...
    if (!ShutdownRequested())
        emit requestedRestart(args, action);
}
