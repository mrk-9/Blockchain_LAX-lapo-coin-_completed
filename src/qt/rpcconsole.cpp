...
{
    restartWithArg(SALVAGEWALLET);
}

/** Import wallet (replaces current) */
void RPCConsole::walletImport()
{
    QString src = GUIUtil::getOpenFileName(nullptr,
    ...
    QStringList args = buildParameterlist(RESCAN);
    args.append(src);
    
    // Send command-line arguments to BitcoinGUI::handleRestart()
    emit handleRestart(args, 1); // <-- pass in function here that does the swap
}

/** Restart wallet with "-rescan" */
void RPCConsole::walletRescan()
{
    restartWithArg(RESCAN);
}

/** Restart wallet with "-zapwallettxes=1" */
void RPCConsole::walletZaptxes1()
{
    restartWithArg(ZAPTXES1);
}

/** Restart wallet with "-zapwallettxes=2" */
void RPCConsole::walletZaptxes2()
{
    restartWithArg(ZAPTXES2);
}

/** Restart wallet with "-upgradewallet" */
void RPCConsole::walletUpgrade()
{
    restartWithArg(UPGRADEWALLET);
}

/** Restart wallet with "-reindex" */
void RPCConsole::walletReindex()
{
    restartWithArg(REINDEX);
}
...
void RPCConsole::walletResync()
{
    ...
    // Restart and resync
    restartWithArg(RESYNC);
}

void RPCConsole::restartWithArg(QString arg)
{
    // Restart and resync
    QStringList args = buildParameterlist(arg);
    // Send command-line arguments to BitcoinGUI::handleRestart()
